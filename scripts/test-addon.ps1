...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlSam-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlSam tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlSam tests" $command
} else {
	Write-Step "Configuring ofxGgmlSam tests"
	Invoke-CheckedNative "cmake configure ofxGgmlSam tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlSam tests"
	Invoke-CheckedNative "cmake build ofxGgmlSam tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlSam tests"
	Invoke-CheckedNative "ctest ofxGgmlSam tests" {
