...
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") 'setStatus\("point prompts cleared"\)' "point example true point clearing"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "getControlPanelRect" "point example separate control panel layout"
Assert-FileContains (Join-Path $exampleRoot "src\ofApp.cpp") "std::min\(\{" "point example no-upscale image layout"
...
Assert-FileContains (Join-Path $scriptRoot "build-sam3-cpp.ps1") "ofxggml-core-cuda-source" "isolated Core ggml CUDA source"
Assert-FileContains (Join-Path $scriptRoot "build-sam3-cpp.ps1") "CheckOnly" "ggml compatibility check mode"
Assert-FileContains (Join-Path $scriptRoot "build-sam3-cpp.ps1") "explicitly opt into -BundledGgml" "explicit bundled ggml fallback"
Assert-FileContains (Join-Path $scriptRoot "build-sam3-cpp.ps1") "-UMATH_LIBRARY" "stale ggml math-library cache reset"
